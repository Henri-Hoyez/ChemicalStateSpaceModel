...
def make_time_shift(df, shift):
    in_cols = ["in_c1", "in_c2", "material velocity"]
    df[in_cols] = df[in_cols].shift(-shift)
    df = df.loc[:df.shape[0]- shift- 1 ]

    # Reset index to make the dataset index begins at 0.
...
def main():
    ...
    df_ssm_dataset = make_time_shift(df_ssm_dataset, strength)

    df_ssm_dataset.to_hdf(f"{args.save_path}/{strength}.h5", key='data')
