...
def h(x, _strength):
    # deviation_from_linear = np.sin(2*np.pi*_strength/40*x)*2
    deviation_from_linear = np.sin(2*np.pi*_strength*x)
    return deviation_from_linear + x
...
def main():
    ...
    outputs = h(outputs, strength)

    df_ssm_dataset = make_dataset(c1_in, c2_in, outputs, labels, vc)

    df_ssm_dataset.to_hdf(f"{args.save_path}/{strength:0.3f}.h5", key='data')
